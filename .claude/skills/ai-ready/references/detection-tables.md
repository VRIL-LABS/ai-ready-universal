...
- **Step 3** (copilot-instructions.md / CLAUDE.md) — include lesson structure conventions: expected folder contents, naming patterns, how to add a new lesson. If lessons have quizzes or assignments, document the expected structure (e.g., each lesson needs `README.md` + `assignment.md` + `solution/`).
...
| CLAUDE.md | exists / missing | repo root |
...
| `copilot-instructions.md` / `CLAUDE.md` | New conventions from recent PR reviews? Maintenance matrix still covers current file relationships? Do the two files still say the same thing? |
