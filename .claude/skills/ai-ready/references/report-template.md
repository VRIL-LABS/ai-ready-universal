...
_Include this section only if the repo already has AI configuration (copilot-instructions.md, CLAUDE.md, custom agents, custom skills). Omit it entirely if there is no pre-existing AI config._

| Asset | Detail |
|-------|--------|
| {asset-name} | {detail — e.g., "542 lines — components, testing, shims, docs"} |
| {.github/agents/} | {count} agents: {names} |
| {.github/skills/} / {.claude/skills/} | {count} skills: {names} |
...
3. **Existing AI config** — if detected (copilot-instructions.md, CLAUDE.md, custom agents/skills)
