...
```
just generate AGENTS.md and CLAUDE.md
```
...
These mined conventions go directly into `copilot-instructions.md` and/or `CLAUDE.md`. The next AI-generated PR follows those rules automatically — regardless of which agent wrote it. You stop repeating yourself.
...
| **`.github/copilot-instructions.md`** / **`CLAUDE.md`** | Coding conventions for Copilot (Chat, completions, PR reviews, CLI) and/or Claude Code. Includes a maintenance matrix of what to update when code changes |
...
| **Copilot code review** (you enable this, Copilot-specific) | Convention violations, missing docs/tests, maintenance matrix gaps | Copilot reads `copilot-instructions.md` (generated by this skill) and reviews PRs against your conventions |
...
Claude Code has no equivalent automatic PR-review trigger, but `CLAUDE.md` (generated alongside `copilot-instructions.md`) is read at the start of every Claude Code session, so the same conventions and maintenance matrix guide any Claude Code contribution or manual review from the first prompt.

## Tested Against

This skill has been validated against a diverse set of repos — courses, applications, VS Code extensions, monorepos, and more. See [.claude/skills/ai-ready/references/training-repos.md](.claude/skills/ai-ready/references/training-repos.md) for the full list.
