...
The AI-Ready skill prepares your repository for effective collaboration with AI coding agents — Claude Code, GitHub Copilot, and others that support the Agent Skills format — by generating a set of structured assets. These assets work together through three complementary mechanisms — each serving a different purpose, read at a different time, and targeting a different aspect of how AI understands your project.
...
**What it is:** A markdown file placed at the root of your repository. It is the one file nearly every AI coding agent reads automatically — Claude Code, the Copilot coding agent (cloud agent), Cursor, and others that support the shared `AGENTS.md` convention.

**When it's read:** Every time an agent starts working on a pull request, issue, or task in your repository. The agent reads this file before it writes any code — it's the first thing it sees.
...
**Think of it as:** The "new hire onboarding doc" for the AI. Just as you'd give a new developer a document explaining how the project works, what to build first, and how to ship code — AGENTS.md does the same for any coding agent that reads it.

**Why it matters:** Without AGENTS.md, a coding agent has to infer project structure from file names and code alone. With it, the agent knows exactly how to build, test, and contribute to your project from the start — regardless of which agent it is.

---

### copilot-instructions.md / CLAUDE.md — Coding Conventions for All Copilot / Claude Code

**What it is:** A repository-level instructions file — `.github/copilot-instructions.md` for Copilot, `CLAUDE.md` at the repo root for Claude Code — automatically injected into every interaction with that agent. The skill generates both by default so either agent picks up the same conventions.

**When it's read:** Every time anyone uses that agent in this repo. For Copilot: Chat, code completions, pull request reviews, CLI, or any other Copilot surface — it applies to all contributors, not just the coding agent. For Claude Code: every session, from the first prompt.
...
**Think of it as:** The "style guide" that is enforced automatically. Instead of hoping every developer reads the style guide, the agent reads it every time and follows it in every suggestion.

**Why it matters:** This ensures consistency across all AI-assisted code — whether it's an inline completion, a Chat response, or a full PR from a coding agent. Every interaction respects the same conventions, and the two files are generated from the same source content so they don't drift apart.

---

### Agent Skills (.github/skills/, .claude/skills/, .agents/skills/) — On-Demand Task Recipes

**What it is:** A directory of markdown files, each containing a step-by-step procedure that can be invoked by name. `.github/skills/`, `.claude/skills/`, and `.agents/skills/` are equivalent, spec-recognized locations — Copilot reads the first, Claude Code the second, and both (plus other Agent-Skills-compatible tools) recognize the third.

**When it's read:** Only when a user explicitly invokes a skill (e.g., "use the add-feature skill" or "follow the scaffolding skill"). Skills are not read automatically — they are pulled in on demand, by whichever agent supports the format.
...
| `copilot-instructions.md` / `CLAUDE.md` | Coding conventions | Automatic (every interaction with that agent) | All users of that agent |
| `.github/skills/` / `.claude/skills/` / `.agents/skills/` | Task-specific procedures | Manual (user invokes by name) | Whoever invokes the skill |

Together, they form a layered system:

1. **AGENTS.md** gives the AI the big picture — what the project is and how it works.
2. **copilot-instructions.md / CLAUDE.md** defines the rules — how code should be written here.
...
### 3. copilot-instructions.md and/or CLAUDE.md

The coding conventions file(s) for all interactions with that agent — `.github/copilot-instructions.md` for Copilot, `CLAUDE.md` at the repo root for Claude Code. Generated together by default from the same conventions content. Includes language idioms, framework patterns, test conventions, code style rules, and the maintenance matrix.
...
A cross-reference table embedded in `copilot-instructions.md` and/or `CLAUDE.md` that maps "when X changes, update Y." This is one of the most valuable assets — it ensures that when code changes, the related documentation, tests, templates, and CI configuration all stay in sync. See [The Maintenance Matrix](#the-maintenance-matrix) section below for details.
...
The skill instructs the agent to scan your repository for:
...
- Is there a `.github/copilot-instructions.md` and/or `CLAUDE.md`?
- Are there existing skills in `.github/skills/`, `.claude/skills/`, or `.agents/skills/`?
...
- A **Go microservice** gets copilot-instructions/CLAUDE.md content about error handling patterns, interface conventions, and module structure — not React component guidelines.
...
The maintenance matrix is embedded in `.github/copilot-instructions.md` and/or `CLAUDE.md`, which means it is automatically included in every interaction with that agent. When the agent suggests a change to a file in the "when this changes" column, it knows to also suggest or remind about the corresponding updates.
...
- **For Copilot Chat or a Claude Code session:** When discussing a change, the agent can reference the matrix to remind you what else needs updating.
